...
module.exports = {
    ...
    checkStock: function (error, item, quantity) {
        if (error) {
        } else {
            this.connection.query("SELECT stock_qty, price, product_name FROM products", function (err, stock_res) {
                if (err) {
                    ...
                    if (quantity === undefined) {
                        ...
                        console.log(price[index].product_name + " available. Price per unit is: $" + price[index].price.toFixed(2) + "\n\nHere's your quote: $" + total.toFixed(2) + "\n");
                    }
                }
            });
            this.connection.end();
